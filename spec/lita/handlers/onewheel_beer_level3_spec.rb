...
describe Lita::Handlers::OnewheelBeerLevel3, lita_handler: true do
  before do
    mock = File.open('spec/fixtures/level3.json').read
  ...
  it 'runs a random beer through' do
    send_command 'level3 roulette'
    expect(replies.count).to eq(1)
    expect(replies.last).to include("Bailey's tap")
  end

  it 'runs a random beer through' do
    send_command 'level3 random'
    expect(replies.count).to eq(1)
    expect(replies.last).to include("Bailey's tap")
  end

  it 'searches with a space' do
    send_command 'level3 zeus juice'
